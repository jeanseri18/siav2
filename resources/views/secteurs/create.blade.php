...
@section('page-title', 'Ajouter un Secteur')
    ...
    <div class="app-card">
        <div class="app-card-header">
            <h2 class="app-card-title">
                <i class="fas fa-map-marker-alt me-2"></i>Ajouter un Secteur
            </h2>
        </div>
        
        <div class="app-card-body">
            <form action="{{ route('secteurs.store') }}" method="POST" class="app-form">
                @csrf
                ...
                <div class="app-form-group">
                    ...
                    <select name="quartier_id" id="quartier_id" class="app-form-select" required>
                        <option value="" disabled selected>-- Sélectionner un quartier --</option>
                        @foreach($quartiers as $quartier)
                            <option value="{{ $quartier->id }}">{{ $quartier->nom }} - {{ $quartier->commune->nom }} ({{ $quartier->commune->ville->nom }})</option>
                        @endforeach
                    </select>
